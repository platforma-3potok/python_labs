...
def csv_to_json(csv_path: str, json_path: str) -> None:
    ...
    try:
        ...
        with open(csv_file, "r", encoding="utf-8") as f:
            ...
            if not has_header:
                raise ValueError(
                    "CSV файл, вероятно, не содержит заголовок (эвристический подход)"
                )
...
try:
    json_to_csv(
        "python_labs/data/samples/empty_json.json",
        "python_labs/data/out/empty_json.csv",
    )
    csv_to_json(
        "python_labs/data/samples/no_header_csv.csv",
        "python_labs/data/out/no_header_csv.json",
    )
    json_to_csv(
        "python_labs/data/samples/test_missing_fields.json",
        "python_labs/data/out/missing_fields_from_json.csv",
    )
    json_to_csv(
        "python_labs/data/samples/people.json",
        "python_labs/data/out/people_from_json.csv",
    )
    csv_to_json(
        "python_labs/data/samples/people.csv",
        "python_labs/data/out/people_from_csv.json",
    )
except Exception as e:
    print(f"Ошибка: {e}")
